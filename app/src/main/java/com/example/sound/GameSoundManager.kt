...
class GameSoundManager(private val context: Context) {
    ...
    fun pauseBackgroundMusic() {
        synchronized(this) {
            try { musicPlayer?.pause() } catch (_: Throwable) {}
    ...
    private fun startMusicStream() {
        ...
        try {
            musicPlayer?.release()
            musicPlayer = MediaPlayer.create(context, com.example.R.raw.background_music)?.apply {
                isLooping = true
                setVolume(0.20f, 0.20f)
                start()
            }
            isPlayingMusic = musicPlayer != null
        } catch (_: Throwable) {
            musicPlayer = null
            isPlayingMusic = false
        }
